...
export default function RestaurantHero({ restaurantData, UpdRestaurant, DelRestaurant, VerifyRestaurant }) {
    ...
    const HandleReserve = () => {
        const payload = {
            from: uid,
            message: `Request to reserve a seat on dd-mm-yyyy`,
            isRead: false,
        }
        axios.post(`/users/${restaurantData.owner}/notifications`, payload)
            .then((res) => {
                toast.success("A message has been sent to the owner!");
            })
            .catch((err) => {
    ...
    return (
        <div className="flex flex-col items-center lg:flex-row bg-zinc-800 p-6 rounded-xl border border-zinc-700 shadow-xl gap-6">
            ...
            <div className="flex flex-col space-y-2 text-zinc-300 text-left flex-grow w-full">
                ...
                <button className="self-start inline-block border-2 text-black rounded-lg bg-sky-400 hover:bg-yellow-300 px-3 py-1 transition-colors duration-150" onClick={HandleReserve}> {restaurantData.reserveSeat} </button>
